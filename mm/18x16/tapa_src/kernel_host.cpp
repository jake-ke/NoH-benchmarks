...
//  data_t A[I][K], B[K][J], C[I][J], C_golden[I][J]; 
  ...
  for (int i = 0; i < I; i++) 
    for (int k = 0; k < K; k++) {
#ifdef LAYOUT2      
      A[i][k] = (data_t)((rand() / (float)RAND_MAX) * 255);
#endif
#ifdef LAYOUT3      
      A[k][i] = (data_t)((rand() / (float)RAND_MAX) * 255);
#endif      
    }
  ...
  for (int j = 0; j < J; j++)
    for (int k = 0; k < K; k++) {
#ifdef LAYOUT2      
      B[j][k] = (data_t)((rand() / (float)RAND_MAX) * 255);
#endif
#ifdef LAYOUT3      
      B[k][j] = (data_t)((rand() / (float)RAND_MAX) * 255);
#endif      
    }

  {
    ...
    std::vector<short, tapa::aligned_allocator<short>> dev_A(18432);
    ...
    // Launch the kernel
    tapa::invoke(kernel0, FLAGS_bitstream, buffer_A[0].vectorized<32>(), buffer_B[0].vectorized<32>(), buffer_C[0].vectorized<32>());
