// ==================================================
// RTL generated by RapidStream
//
// Copyright 2024 RapidStream Design Automation, Inc.
// All Rights Reserved.
// ==================================================
`timescale 1 ns / 1 ps
/**   Generated by RapidStream   **/
module __rs_pipelined_fifo_fifo_C_drain_C_drain_IO_L1_out_12_12 #(
    parameter DATA_WIDTH = 32,
    parameter ADDR_WIDTH = 5,
    parameter DEPTH      = 32
) (
    input wire                       clk,
    input wire  [(DATA_WIDTH - 1):0] if_din,
    output wire [(DATA_WIDTH - 1):0] if_dout,
    output wire                      if_empty_n,
    output wire                      if_full_n,
    input wire                       if_read,
    input wire                       if_read_ce,
    input wire                       if_write,
    input wire                       if_write_ce,
    input wire                       reset
);

wire        __rs_pipelined_reset;



fifo #(
    .ADDR_WIDTH (ADDR_WIDTH),
    .DATA_WIDTH (DATA_WIDTH),
    .DEPTH      (DEPTH)
) _ /**   fifo_C_drain_C_drain_IO_L1_out_12_12   **/ (
    .clk         (clk),
    .if_din      (if_din),
    .if_dout     (if_dout),
    .if_empty_n  (if_empty_n),
    .if_full_n   (if_full_n),
    .if_read     (if_read),
    .if_read_ce  ( 1'b1),
    .if_write    (if_write),
    .if_write_ce ( 1'b1),
    .reset       (__rs_pipelined_reset)
);

(* RS_ROUTE="SLOT_X0Y0_TO_SLOT_X0Y0,SLOT_X0Y1_TO_SLOT_X0Y1,SLOT_X1Y1_TO_SLOT_X1Y1,SLOT_X1Y2_TO_SLOT_X1Y2,SLOT_X1Y3_TO_SLOT_X1Y3" *)
__rs_ff_pipeline_8 #(
    .BODY_LEVEL      (8),
    .DATA_WIDTH      (1),
    .HEAD_LEVEL      (1),
    .TAIL_LEVEL      (1),
    .__BODY_0_REGION ("SLOT_X0Y0_TO_SLOT_X0Y0"),
    .__BODY_1_REGION ("SLOT_X0Y1_TO_SLOT_X0Y1"),
    .__BODY_2_REGION ("SLOT_X0Y1_TO_SLOT_X0Y1"),
    .__BODY_3_REGION ("SLOT_X1Y1_TO_SLOT_X1Y1"),
    .__BODY_4_REGION ("SLOT_X1Y1_TO_SLOT_X1Y1"),
    .__BODY_5_REGION ("SLOT_X1Y2_TO_SLOT_X1Y2"),
    .__BODY_6_REGION ("SLOT_X1Y2_TO_SLOT_X1Y2"),
    .__BODY_7_REGION ("SLOT_X1Y3_TO_SLOT_X1Y3"),
    .__HEAD_REGION   ("SLOT_X0Y0_TO_SLOT_X0Y0"),
    .__TAIL_REGION   ("SLOT_X1Y3_TO_SLOT_X1Y3")
) ff_0 /**   Generated by RapidStream   **/ (
    .clk     (clk),
    .if_din  ({ reset }),
    .if_dout ({ __rs_pipelined_reset })
);

endmodule  // __rs_pipelined_fifo_fifo_C_drain_C_drain_IO_L1_out_12_12